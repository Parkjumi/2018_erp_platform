        ...
        <v-flex xs12>
          <v-data-table
            ...
            <template slot="items" slot-scope="props">
              <tr>
                 <td @click="$router.push('/notice/delivery/detail/'+props.item.id)">{{props.index + 1}}</td>
...
<script>
  ...
  export default {
    ...
    methods: {
      initPurchaseData() {
        this.$axios.get('http://freshntech.cafe24.com/deboarders')
        .then(res => {
      ...
      moveAppend() {
        this.$router.push('/notice/delivery/append');
      }
    },
    created() {
      setTimeout(()=>{
